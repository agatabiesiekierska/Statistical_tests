...
class tStudent:
    ...
    def Show_Info(self):
        print('t-student test'.upper().center(60))
        print('='*60)
        print(f'Path to file:\t {self.path}')
        print(f'Number of elements: {len(self.data)}')
        print(f'Confidence level:\t\t\t {self.significance}')
        print(f'Mean value:\t {self.values[0]}')
        print(f'Standard deviation value:\t {round(self.values[1],8)}')
        print(f't-critical value:\t {self.t_kryt}')
        print(f't-value:\t {round(self.values[2],6)}')
        print(f'If t > t_crit:\t {self.values[3]}')
        if self.values[3] == True: print('The results differ at a statistically significant level!')
        else: print('The results do not differ at a statistically significant level!')
        print('='*60)
    ...
    def Export_to_file(self,path_to_export):
        with open(path_to_export,'a') as file:
            ...
            file.write('='*60 + '\n')
            file.write(f'Path to file:\t {self.path}\n')
            file.write(f'Number of elements: {len(self.data)}\n')
            file.write(f'Confidence level:\t\t\t {self.significance}\n')
            file.write(f'Mean value:\t {self.values[0]}\n')
            file.write(f'Standard deviation value:\t {round(self.values[1],8)}\n')
            file.write(f't-critical value:\t {self.t_kryt}\n')
            file.write(f't-value:\t {round(self.values[2],6)}\n')
            file.write(f'If t > t_crit:\t {self.values[3]}\n')
            if self.values[3] == True: file.write('The results differ at a statistically significant level!\n')
            else: file.write('The results do not differ at a statistically significant level!\n')
            file.write('='*60 + '\n')
