class DixonTest:
    ...
    def Comparison(self):
        ...
        for i,j in zip(self.values, val):
            if i > self.q_kryt:
                compare = f'The value {j} should be discarded!'
            else: compare = f'Value {j} should not be rejected!'
            list_compare.append(compare)
        return list_compare

    def Show_Info(self):
        print('q dixon\'s test'.upper().center(60))
        print('='*60)
        print(f'Path to file:\t {self.path_to_imp}')
        print(f'Number of elements: {len(self.data)}')
        print(f'Confidence level:\t\t\t {self.significance}')
        print(f'Q_critical value:\t {self.q_kryt}')
        print(f'Gap value:\t {round((max(self.data) - min(self.data)),6)}')
        print(f'Values Q1 i Qn:\t {self.values}')
    ...
    def Export_to_file(self,path_to_export):
        with open(path_to_export,'a') as file:
            file.write('q dixon\'s test'.upper().center(60) + '\n')
            file.write('='*60 + '\n')
            file.write(f'Path to file:\t {self.path_to_imp}\n')
            file.write(f'Number of elements: {len(self.data)}\n')
            file.write(f'Confidece level:\t\t\t {self.significance}\n')
            file.write(f'Q_critical value:\t {self.q_kryt}\n')
            file.write(f'Gap value:\t {round((max(self.data) - min(self.data)),6)}\n')
            file.write(f'Values Q1 i Qn:\t {self.values}\n')
